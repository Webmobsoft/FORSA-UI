...
<div class="wrapper">
    <div class="structure-row">
        ...
        <div class="right-sec">
		   ...
		   if(isset( $_GET['id']))
		   {
			   $select_closed_loan = mysql_query("select tbl_market_request.*, tbl_users.fname, tbl_users.lname from tbl_market_request inner join tbl_users on tbl_users.id = tbl_market_request.borrower_id where tbl_market_request.status='open' AND tbl_market_request.id ='".$_GET['id']."'");
             
             
		   }
		    else
			{
                $select_closed_loan = mysql_query("select tbl_market_request.*, tbl_users.fname, tbl_users.lname from tbl_market_request inner join tbl_users on tbl_users.id = tbl_market_request.borrower_id where tbl_market_request.status='open'");
                        ...
                        <div class="col-xs-12">
                            <div class="sec-box">
                                ...
                                <header style="height:70px;">
                                    <h2 class="heading" style="float:left">Bids</h2>                                        									<a href="seeMMOffers.php"><input placeholder="" name="goToOffers" value="SEE OFFERS" class="btn btn-primary style2" type="submit" style="float:right;margin-right:10%;"></a>										
                                </header>
                                <div class="contents">
								...
								while ($rows = mysql_fetch_array($select_closed_loan)) {
										...
										?>
                                                        <tr class="gradeX">
                                                            ...
                                                            <td class=" "><?php echo $rows['min_bid']; ?>%</td>
                                                            <td class="a-right a-right "><?php echo date('j F Y', strtotime($rows['maturity'])); ?></td>
                                                            
                                                           <?php if(!isset( $_GET['id'])){ ?>   <td class=" "><a href="mmOpen_loan.php?id=<?php echo $rows['id']; ?>"><i class="fa fa-eye fa-2x"></i></a></td><?php } ?>
                                                        </tr>
        <?php
    }
    ?>
