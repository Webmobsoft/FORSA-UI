...
<?php  if(isset($_POST['UPDATE']))
{
	...
	for ($x = 0 ; $x <= $totalTabs ; $x++ )
	{
		 ...
		 $update = mysql_query("UPDATE tbl_money_market SET `eur_start_date` = '".$startdate."' , `eur_value_date` = '".$value_date_manual[$x]."' , `eur_maturity_date` = '".$mat_date_manual[$x]."' WHERE `id` = '".$id."'");
                ...
                <div class="container-liquid">
                    <div class="row">
                        <div class="col-xs-12">
                            ...
                            <option value="chf">CHF </option><option value="eur" checked="checked" selected>EUR </option><option value="usd" >USD </option></select>		
				</span>
                                    <?php 
                                     $selectShowStatus = mysql_query("SELECT `eur_showDatesManual` FROM `tbl_money_market` WHERE `id` = '1'");
                                     $showStts = mysql_fetch_array($selectShowStatus);

                                    ?>
                                
                                <header style="height:60px;">
                                    <h2 class="heading" style="float:left;">EUR Money Market</h2>
                                     <span style="float:left;margin-left:75px;">
					<h6> <input type="radio" name = "radioCheck" title="Show Dates Automatic" class="radio-inline" value ="n" <?php if( $_SESSION['privilege'] != "1"  ){ ?> onclick="setMaturityShow(this.value)" <?php } ?><?php if($showStts ['eur_showDatesManual'] == 'n') {echo  'checked'; } ?>> Show Dates Automatic
					<input type="radio" name = "radioCheck" title="Show Dates Manually" class="radio-inline" value="y" <?php if( $_SESSION['privilege'] != "1"  ){ ?> onclick="setMaturityShow(this.value)" <?php } ?> <?php if($showStts ['eur_showDatesManual'] == 'y') {echo  'checked'; } ?>> Show Dates Manually</h6>
					<input type="hidden" name="choosenType" id = "choosenType"value="<?php echo $showStts['eur_showDatesManual']; ?>"/>
				     </span>
                                </header>
                                <div class="contents">
							...
							if($rows['id'] !='15') {
                                                        ...
                                                        <tr class="gradeX">
														   ...
														   <td class="" onclick="" style="cursor: pointer;display:none;"><?php echo $rows['id']; ?></td>
                                                            ...
                                                            <td class="showManual"  style="display:none;"><input title="Add Start Date" type="text" placeholder="" id="start_date_manual<?php echo $count1;  ?>"   name="start_date_manual[]" value = "<?php if( $rows['eur_start_date'] != ""){ echo $rows['eur_start_date'] ; }else { echo $start_date; } ?>" class="form-control" readonly>          </td>
															<td class ="showManual" style="display:none;"><input title="Add value Date" type="text" placeholder="" id="value_date_manual<?php echo $count1;  ?>" name="value_date_manual[]" value="<?php if( $rows['eur_value_date'] != ""){ echo $rows['eur_value_date']; }else{ echo $valueDate; } ?>" class="form-control" readonly></td>
															<td class="showManual"  style="display:none;"><input title="Add Maturity Date" type="text" placeholder="" id="mat_date_manual<?php echo $count1;  ?>" name="mat_date_manual[]" value="<?php if( $rows['eur_maturity_date'] != ""){ echo $rows['eur_maturity_date']; }else { echo $maturityDay; } ?>" class="form-control" readonly></td>
	...
	else
	{
		...
		$(".showManual").hide();
                                     ...
                                     function setMaturityShow(value)
									{
										...
										var data_to_send = 'status=' + value + '&function=eur_setMaturityShow';
