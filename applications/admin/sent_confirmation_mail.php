...
<div class="wrapper">
    <div class="structure-row">
        ...
        <div class="right-sec">
            ...
            // exit();
            $select_lender = mysqli_query($mysql_connect,"SELECT tbl_confirmation_email.*, tbl_users.fname, tbl_users.lname  FROM `tbl_confirmation_email` inner join tbl_users on tbl_users.id = tbl_confirmation_email.to where tbl_confirmation_email.status = 'Y'");
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="sec-box">
                                ...
                                <header>
                                    <h2 class="heading">Sent Confirmation Mail</h2>
                                </header>
                                <div class="contents">
...
//                                                echo '</pre>';
                                                        ...
                                                        <tr class="gradeX">
                                                          ...
                                                          <td><a href="confirmation_email_details.php?id=<?php echo $rows['id'];?>"><?php echo $rows['subject']." - ".$small = substr($rows['message'], 0, 25)."....";  ?></a></td>
                                                            <td><?php echo date('d/m/Y H:i:s A', strtotime($rows['sent_at'])); ?></td>
                                                            <td><a class="deleteconfirmationEmail" data-id=<?php echo $user_id;?> href="javascript:void(0);"><i class="fa fa-times fa-2x red"></i></a></td>
                                                        </tr>
                                                            <?php
                                                        }
                                                        ?>
...
<script>
 ...
 $('.deleteconfirmationEmail').click(function(){
 
 var id = $(this).data('id');
 var obj = $(this);
 var check = confirm('Do you really want to delete this email.');
 if(check == true) {
   var data_to_send = 'id=' + id +'&table=tbl_confirmation_email&function=deleteEmails';
   $.ajax({
