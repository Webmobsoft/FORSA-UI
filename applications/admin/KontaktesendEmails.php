        ...
        <div class="right-sec">
            ...
            if (isset($_POST['submit'])) {              
                if (!empty($_POST['userEmail']) && !empty($_POST['emailMessage']) && !empty($_POST['emailSubject'])) {
                    ...
                    $email = mysqli_query($mysql_connect,"SELECT email FROM `tbl_customers` where id IN (".$userEmail.")");
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="col-md-8">
                                                            <select id="userEmail" name="userEmail[]" class="form-control" multiple="multiple" style="height: 300px;">
                                                                <?php
                                                                    $users = mysqli_query($mysql_connect,"SELECT id,id_i,email, concat((first_name), (' '), (Surname)) as name ,Name_company FROM `tbl_customers` WHERE id_i != '' AND id_i != 'ID' AND processed = 'N'");
                                                                    if(mysqli_num_rows($users) > 0) {
                                                                        while($user = mysqli_fetch_assoc($users)) {
                                                                            if($user['id_i'] != 'ID' && !empty($user['id_i'])){
                                                                            echo '<option value="'.$user['id'].'">'.$user['name'].' - ( '.$user["Name_company"].' )</option>';
                                                                        }
                                                                        }
                                                                    }
                                                                ?>
